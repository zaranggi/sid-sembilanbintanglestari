...
namespace App\Modules\Cicilan\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Routing\Controller;
use App\Modules\Cicilan\Models\Cicilan;
use App\Modules\Booking\Models\Mtran;
use App\Modules\Cicilan\Models\Mtrankonsumen;
...
class CicilanController extends Controller
{
    ...
    public function __construct(Cicilan $data)
    {
        $this->data = $data;
    }
    ...
    public function index()
    {
        ...
        return view('Cicilan::index',['data' => $data]);
    }
    ...
    public function listall($id)
    {
        ...
        return view('Cicilan::data',['data' => $data]);
    }
    ...
    public function detail($id)
    {
        ...
        $his = Mtrankonsumen::where("id_spr","=",$id)
				->where("id_jenis","=","5")->orderBy("id","desc")->get();

		$bank_pt = Bank::All();
		$bank = $this->data->listbank();

        return view('Cicilan::view',['data' => $data,  'his' => $his, 'konsumen' => $konsumen,'bank_pt' => $bank_pt, 'bank' => $bank]);
    }
    ...
    {
        $data = Cicilan::findOrFail($id);

        return view("cicilan::edit")->with('data', $data);
    ...
    public function simpanbayar(Request $request)
    {
        ...
        if ($validator->fails()) {
            return Redirect::to('cicilan')->withErrors($validator)->withInput();
        }
        else
        {
            ...
            $menu = Cicilan::findOrFail($request->input("id_tagihan"));
			//if(($gross + $menu->bayar) <= $menu->tagihan){
				...
				$data->kode 	        = "CC-".$menu->id."-".$menu->urutan;
				...
				$data->save();
					...
					//=================Insert Log Konsumen===================================
						$kon = Trxkonsumen::findOrFail($menu->id_spr);
							...
							$k->status = "Cicilan";
							$k->keterangan = "Cicilan : ".$menu->urutan." - Nominal = ".$gross;
				 ...
				 //=================Insert Mtran ===================================
					...
					$mt->rtype			= "AR";
					$mt->kode_tagihan 	= "CC-".$menu->id."-".$menu->urutan;
					...
					$ket_jurnal = "Penerimaan UM/Pembayaran Cicilan :: ".$mproperti->nama." Kav ".$munit->nama." . Dari :".$request->input('nama')." Kode: BOK-".$menu->id."-".$menu->urutan." | ".$request->input("keterangan");
			...
			//}else{
			//	Session::flash('flash_message', 'Pembayaran Harus sesuai dengan Tagihan Cicilan!');
			//}
            return redirect('cicilan/detail/'.$menu->id_spr);
	...
	public function cetak($kode)
    {
		...
		$pdf = PDF::loadview('Cicilan::cetak', ['data'=>$data])->setPaper('a4');
