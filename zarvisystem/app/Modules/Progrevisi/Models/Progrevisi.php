<?php
namespace App\Modules\Progrevisi\Models;
 ...
 */
class Progrevisi extends Model {
    ...
    public function dataall()
	{
        ...
        $listuser = $db->select("
						SELECT a.*, b.bobot,b.ach , d.judul, c.nama as nama_properti,
						e.nama as nama_kav,
						e.tipe as tipe_unit
						FROM spk_proyek a
						LEFT JOIN (
	...
	public function detail($id,$jenis_spk)
	{
		$db = DB::connection('mysql');
        $listuser = $db->select("
				select progres_proyek.*,jenis_spk.nama_spk, jenis_progres_kategori.nama as nama_pekerjaan
				from `progres_proyek`
				left join `jenis_spk` on `progres_proyek`.`jenis_spk` = `jenis_spk`.`id`
				left join `jenis_progres_kategori` on `progres_proyek`.`id_pekerjaan` = `jenis_progres_kategori`.`id`
