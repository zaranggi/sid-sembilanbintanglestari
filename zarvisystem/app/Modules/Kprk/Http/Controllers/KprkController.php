...
namespace App\Modules\Kprk\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Routing\Controller;
use App\Modules\Kprk\Models\Kprk;
use App\Modules\Booking\Models\Mtran;
use App\Modules\Pomaterial\Models\Tconst;
use App\Modules\Kprk\Models\Mtrankonsumen;
...
class KprkController extends Controller
{
    ...
    public function __construct(Kprk $data)
    {
        $this->data = $data;
    }
    ...
    public function index()
    {
        ...
        return view('Kprk::index',['data' => $data]);
    }
    ...
    public function listall($id)
    {
        ...
        return view('Kprk::data',['data' => $data]);
    }
    ...
    public function detail($id)
    {
        ...
        $his = Mtrankonsumen::where("id_spr","=",$id)
				->where("id_jenis","=","7")->orderBy("id","desc")->get();
		$bank_pt = Bank::All();
		$bank = $this->data->listbank();
		
        return view('Kprk::view',['data' => $data, 'his' => $his, 'konsumen' => $konsumen,'bank_pt' => $bank_pt, 'bank' => $bank]);
    }
    ...
    public function simpanbayar(Request $request)
    {
        ...
        if ($validator->fails()) { 
            return Redirect::to('kprk')->withErrors($validator)->withInput();  
        }
        else
        {
            ...
            $menu = Kprk::findOrFail($request->input("id_tagihan")); 
			if(($gross + $menu->bayar) <= $menu->tagihan){			
				...
				$data->kode 	        = "KPRK-".$menu->id."-".$menu->urutan;
				...
				$data->save();
					...
					//=================Insert Log Konsumen===================================
						...
						$k->status = "KPRK"; 
						$k->keterangan = "KPRK : Nominal = ".$gross; 
					...
					$mt->rtype			= "AR";
					$mt->kode_tagihan 	= "KPRK-".$menu->id."-".$menu->urutan;
					...
					$ket_jurnal = "Penerimaan Pembayaran Kekurangan KPR :: ".$mproperti->nama." Kav ".$munit->nama." . Dari :".$request->input('nama')." Kode: BOK-".$menu->id."-".$menu->urutan." | ".$request->input("keterangan");
	...
	public function cetak($kode)
    {
		...
		$pdf = PDF::loadview('Kprk::cetak', ['data'=>$data])->setPaper('a4');
