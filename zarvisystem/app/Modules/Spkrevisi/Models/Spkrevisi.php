...
class Spkrevisi extends Model {
    ...
    public function jenis_spk(){ 
		...
		$db = DB::connection('mysql');
        $spk = $db->select("SELECT a.*, b.nama as nama_properti FROM mrab_proyek a left join properti b on a.id_properti = b.id where a.kategori='Revisi' and a.status = 2 ");
