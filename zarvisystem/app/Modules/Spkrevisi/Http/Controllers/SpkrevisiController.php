...
namespace App\Modules\Spkrevisi\Http\Controllers;
...
use App\Modules\Spkrevisi\Models\Terminrevisi; 
use App\Modules\Spkrevisi\Models\Progresbangun; 
...
class SpkrevisiController extends Controller
{  /**
  ...
  */
	...
	public function __construct(Spkrevisi $data)
	{
		$this->data = $data;
	}
   ...
   public function index()
   {
       ...
       return view('Spkrevisi::index',['data' => $data]);
   }
    ...
    public function create()
    {
        ...
        return view('Spkrevisi::create', ['dirman' => $dirman, 'subkon'=> $subkon, 'jenis_spk' =>$jenis_spk]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('spkrevisi/create')->withErrors($validator)->withInput();
 
        } else {
			
			$cek = Mrabr::findOrfail($request->input("jenis"));
			
            $noUrutAkhir = Spkrevisi::max('id');
            $kode = "SPKP-".sprintf("%04s", $noUrutAkhir + 1);

            $data = new Spkrevisi;
            $data->id_properti = $cek->id_properti;
            $data->id_kav = $cek->id_kav;
			...
			$mrabjob = Mrabrjob::where("id_mrabp","=",$request->input("jenis"))
								->get();
    ...
    {
       $data = Spkrevisi::findOrfail($idspk);
       $data->status = 1;
       $data->save();
    ...
    public function detail($id)
    {
        ...
        return view('Spkrevisi::detail',['data' =>$data, 'termin' => $termin, 'job' => $job]);
