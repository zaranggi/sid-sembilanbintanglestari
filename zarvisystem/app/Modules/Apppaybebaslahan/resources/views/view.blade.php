...
@foreach($data as $rr)
    ...
    <h1 class="page-header">Data Pembayaran <small> Pembebasan Lahan </small></h1>
        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-heading">
                    <h4 class="panel-title">Data Pembebasan Lahan </h4>
                </div>
                <!-- end panel-heading -->
                <!-- begin panel-body -->
						...
						<div class="invoice-header">
							<div class="invoice-from">
								...
								<address class="m-t-5 m-b-5">
									<strong class="text-inverse">{{ $rr->nama_pemilik }}</strong><br />
									Alamat: {{ $rr->alamat }}<br /> 
									Phone: {{ $rr->hp }}<br /> 
									NIK: {{ $rr->ktp }}<br /> 
								</address>
							...
							<div class="invoice-to">
								...
								<address class="m-t-5 m-b-5">
									<strong class="text-lime">{{ number_format($rr->luas_tanah) }} m<sup>2</sup><br/>
									Harga : Rp {{ number_format($rr->harga) }} / m<sup>2</sup><br/> 
                                    Total : Rp {{ number_format($rr->gross) }}<br/> </strong>
								</address>
							</div> 
							<div class="invoice-to">
								Tanggal Jatuh Tempo
								<address class="m-t-5 m-b-5">
									<strong class="text-lime">{{ App\Helpers\Tanggal::tgl_indo($rr->tanggal_jth_tempo) }} <br/></strong>
								</address>
							</div> 
						</div>
    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-inverse" data-sortable-id="table-basic-1">
                <!-- begin panel-heading -->
                <div class="panel-heading">
                    <h4 class="panel-title">Pembayaran Pembebasan Lahan</h4>
                        ...
                        <table id="data-table"  class="table table-bordered table-hover width-full">
                            <thead>
                                <tr class="text-center"> 
                                    ...
                                    <th class="text-nowrap">Nilai Termin</th>
                                    <th class="text-nowrap">Pembayaran</th>
                                    <th class="text-nowrap">Kekurangan</th>
                             ...
                             <tbody>
                        @foreach($data_termin as $r)

                            <tr>
                                <td >Termin Ke - {{  $r->termin }}</td>
                                <td class="text-right" >{{  number_format($r->nilai) }}</td>
                                <td class="text-right" >{{  number_format($r->jumlah_bayar) }}</td>
                                <td class="text-right" >{{  number_format($r->nilai - $r->jumlah_bayar) }}</td>
                                <td class="text-center">{{  $r->status_pembayaran }}</td>                                
                                <td class="text-center">   
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-158px, 19px, 0px);">
                                            ...
                                            <a href="{{ url('apppaybebaslahan/approve/'. $r->id) }}" data-confirm="Apakah yakin akan Approve SPK?" rel="nofollow" class="dropdown-item text-lime">
                                                <i class="fa fa-save "></i> Approve
                                            </a>                                            
                                            <a href="{{ url('apppaybebaslahan/reject/'. $r->id) }}" data-confirm="Apakah yakin akan Reject SPK?" rel="nofollow" class="dropdown-item text-warning">
	...
	<div class="row">
        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-inverse" data-sortable-id="table-basic-1">
                <!-- begin panel-heading -->
                <div class="panel-heading">
                    <h4 class="panel-title">History Pembayaran Pembebasan Lahan</h4>
                        ...
                        <table class="table table-bordered table-hover width-full">
                            <thead>
                                <tr class="text-center"> 
                                    ...
                                    <th class="text-nowrap">Rek. Penerima</th>
                             ...
                             <tbody>
							 @php $no = 1; @endphp
                        @foreach($data_termin_his as $r)

                            <tr>
                                <td class="text-center text-nowrap">{{ $no }}</td> 
                                <td  class="text-center text-nowrap">{{  App\Helpers\Tanggal::tgl_indo($r->tgl_bayar) }}</td>
                                <td  class="text-nowrap">{{  $r->penerima }}</td>
                                <td class="text-center text-nowrap">{{ $r->tipe_pembayaran }}</td>								
                                <td class="text-nowrap" ><h6><a href="#"> {{ $r->nama_rekening_penerima  }}</a>  <span class="d-block font-size-sm text-muted">{{ $r->bank_penerima ." - ". $r->norek_penerima}}</span>  </h6> </td>
                                <td class="text-right" >{{  number_format($r->jumlah_bayar) }}</td>
                               {{--  <td class="text-center">
                                   
                                    <a href="{{ url('paybebaslahan/cetak/'.$r->id) }}" class="text-lime font-weight-bold" target="_blank">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Pembayaran Pembebasan Lahan</h4>
					<button type="button" class="close text-danger" data-dismiss="modal" aria-hidden="true">x</button>
                </div>
                <div class="modal-body">
                    <form enctype="multipart/form-data" method="POST" action="{{ url("paybebaslahan/simpanbayar")}}">
                        @csrf  
						...
						<div id="tampil" style="display:none;">
							...
							<div class="form-group row m-b-10">
								...
								<div class="col-md-5"> 
									<input type="text" name="norek_penerima"  id="norek_pengirim" class="form-control text-right">                            
								</div>
							</div> 						
							<div class="form-group row m-b-10">
