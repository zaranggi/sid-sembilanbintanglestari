        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-inverse" data-sortable-id="table-basic-1">
                ...
                <div class="panel-heading">
                    <h4 class="panel-title">Pembayaran Reknanan - Material</h4>
                    ...
                    <div class="panel pagination-grey clearfix m-b-0">
                        ...
                        <table id="data-table"  class="table table-bordered table-hover width-full">
                            <thead>
                                <tr class="text-center"> 
                                    ...
                                    <th class="text-nowrap">Nama Rekanan</th>
                                    <th class="text-nowrap" >Alamat</th>
                                    <th class="text-nowrap">Kontak</th>
                                    <th class="text-nowrap">Cara Bayar</th>
                        ...
                        @foreach($data as $r)
                            ...
                            <tr>
                                ...
                                <td  class="text-center text-nowrap">PO-0{{  $r->docno }}</td>
                                <td  class="text-nowrap">{{  $r->nama_rekanan }}</td>
                                <td  class="text-nowrap">{{  $r->alamat }}</td>
                                <td class="text-center text-nowrap">{{ $r->kontak }}</td>
                                <td class="text-center text-nowrap">
                                    @if($r->pembayaran == "Cash")
                                    <span class="label label-theme font-weight-bold">{{ $r->pembayaran }}</span>
                                    @else 
                                    <span class="label label-indigo font-weight-bold">{{ $r->pembayaran }}</span>
                                    @endif
                                </td>
                                <td class="text-right" >Rp {{  number_format($r->gross) }}</td>                            
                                <td class="text-right" >Rp {{  number_format($r->jumlah_bayar) }}</td>       
                                <td class="text-center" >{{  App\Helpers\Tanggal::tgl_indo($r->tgl_bayar) }}</td>                     
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Pembayaran Rekanan - Material</h4>
					<button type="button" class="close text-danger" data-dismiss="modal" aria-hidden="true">x</button>
                </div>
                <div class="modal-body">
                    <form enctype="multipart/form-data" method="POST" action="{{ url("paypomaterial")}}">
                        @csrf  
