...
class Home extends Model {
		...
		$listuser = $db->select("SELECT
					...
					(SELECT
						...
						FROM (
							...
							WHERE id_properti= $id and year(tgl_transaksi) = '$y'
							GROUP BY bulan
							ORDER BY bulan
						) xx
					) AA");
	...
	public function kpr()
	{
		...
		$listuser = $db->select("select log_kpr,
			...
			from konsumen_spr
			where cara_bayar_unit = 'kpr' and status_spr = 1 and year(tgl_transaksi) ='".date("Y")."' ;
		");
	...
	{ 
		...
		$sc = "select a.id, a.nama,ifnull(b.total,0) as total from jenis_status_kav a
		left join (
