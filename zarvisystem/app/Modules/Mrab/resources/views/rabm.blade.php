...
<div id="loader">
    ...
    <h1 class="page-header">RAB Material <small>Pembangunan Rumah </small></h1>
        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-body">                    
					...
					<a href="{{url('mrab') }}" class="btn btn-warning"><i class="fa fa-redo-alt ml-2"></i> Kembali </a>
                    ...
                    <div class="panel pagination-grey clearfix m-b-0">
								...
								<thead>
								 ...
								 @foreach($data as $r)
									...
									<tr> 
										...
										<input type="hidden" name="prdcd[]" value="{{ $r->prdcd }}">{{ $r->prdcd }} </td>
										<td class="text-left text-nowrap">{{ $r->nama }}</td>
										<td class="text-center text-nowrap"><input type="hidden" name="qty[]" value="{{ $r->qty }}">{{ number_format($r->qty) }}</td> 
										<td class="text-center text-nowrap">{{ $r->satuan }}</td> 
										<td class="text-center text-nowrap">{{ number_format($r->price) }}</td> 
										<td class="text-center text-nowrap" id="gg">{{ number_format($r->gross) }}</td>  
						...
						<div class="m-b-15">
							<button type="submit"  id="submit" onclick="return confirm('Apakah Anda Yakin?')" class="btn btn-success btn-block btn-lg">Simpan Rab Material</button>
						</div>
					</form>  
                </div>
            </div>
								...
								<div class="col-md-8">
									<select class="default-select2 form-control datanya" placeholder="Cari / Pilih Material... " name="prdcd">
										@foreach($prodmast as $r)
											<option value="{{$r->prdcd}}|{{$r->nama}}|{{$r->satuan}}|{{$r->price}}">{{ $r->prdcd }} :: {{ $r->nama }} ({{ $r->satuan }})</option>
										@endforeach
									</select>								
								</div>
							</div>
...
<script type="text/javascript"> 
    $(document).ready(function(){
        ...
        $(".add-row").click(function(){
            ...
            var pricev = tandaPemisahTitik(price); 
            var markup = "<tr><td class='text-center'><input type='hidden' name='prdcd[]' value='" + plu + "'>" + plu + "</td><td>" + deskripsi + "</td><td class='text-center'><input type='hidden' name='qty[]' value='" + qtynya + "'>" + qtyv + "</td><td class='text-center'>" + satuan + "</td><td class='text-center'>" + pricev + "</td><td class='text-center' id='gg'>" + tandaPemisahTitik(qtynya * price) + "</td><td><a href='javascript:void(0);' class='delete-row'><i class='fa fa-trash-alt text-danger'></i></a></td></tr>";
