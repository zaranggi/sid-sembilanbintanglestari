<?php
namespace App\Modules\Bayarsubkon\Models;
 ...
 */
class Bayarsubkon extends Model {
    ...
    public function dataall(){ 
      ...
      ->select(db::raw("spk.*,subkon.nama as nama_subkon, properti.nama as nama_properti, properti_kav.nama as nama_kav, properti_kav.tipe as tipe_unit"))
      ->leftjoin("properti","spk.id_properti","=","properti.id")            
      ->leftjoin("properti_kav","spk.id_kav","=","properti_kav.id")          
      ->leftjoin("subkon","spk.id_subkon","=","subkon.id")            
      ->where("spk.status","=","4")
   ...
   public function progres($id,$jenis_spk)
	...
	public function spk($id)
	{
        ...
        $listuser = $db->select("SELECT a.*,b.nama_spk, ((bobot/t) * c.ach) as achiev 
							...
							left join ( 
								select jenis_spk,id_spk, count(*) as t, 
