...
namespace App\Modules\Tambahan\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Routing\Controller;
use App\Modules\Tambahan\Models\Tambahan;
use App\Modules\Booking\Models\Mtran;
use App\Modules\Tambahan\Models\Mtrankonsumen;
...
class TambahanController extends Controller
{
    ...
    public function __construct(Tambahan $data)
    {
        $this->data = $data;
    }
    ...
    {	
        ...
        return view('Tambahan::index',['data' => $data]);
    }
    ...
    public function listall($id)
    {
        ...
        return view('Tambahan::data',['data' => $data]);
    }
    ...
    public function detail($id)
    {
        ...
        $his = Mtrankonsumen::where("id_spr","=",$id)
				->wherein("id_jenis",[3,4])->orderBy("id","desc")->get();
		$bank_pt = Bank::All();
		$bank = $this->data->listbank();		
        return view('Tambahan::view',['data' => $data, 'his' => $his, 'konsumen' => $konsumen,'bank_pt' => $bank_pt, 'bank' => $bank]);
    }
    ...
    public function edit($id)
    {
        ...
        return view("tambahan::edit")->with('data', $data); 
    ...
    public function simpanbayar(Request $request)
    {
        ...
        if ($validator->fails()) { 
            return Redirect::to('tambahan')->withErrors($validator)->withInput();  
        }
        else
        {
            ...
            $menu = Tambahan::findOrFail($request->input("id_tagihan"));      
			if(($gross + $menu->bayar) <= $menu->tagihan){			
				...
				if($menu->id_jenis == 3){
					...
					$data->kode 	        = "TN-".$menu->id."-".$menu->urutan;
					$data->id_jenis 	    = 3;
					...
					$data->save();
						...
						//=================Insert Log Konsumen===================================
							$kon = Trxkonsumen::findOrFail($menu->id_spr);
								...
								$k->status = "Tambahan"; 
								$k->keterangan = "Tambahan : ".$menu->urutan." - Nominal = ".$gross; 
					...
					$data = new Mtrankonsumen;
					$data->kode 	        = "TB-".$menu->id."-".$menu->urutan;
					$data->id_jenis 	    = 4;
					$data->id_spr  			= $menu->id_spr;
					$data->tanggal 	        = $request->input('tanggal');
					$data->pembayar 	    = $request->input('nama');
					...
					$data->save();
						...
						//=================Insert Log Konsumen===================================
							$kon = Trxkonsumen::findOrFail($menu->id_spr);
								...
								$k->created_by = Auth::user()->name; 
								$k->save();
							//=================Insert Log Konsumen===================================//=================Insert Mtran ===================================
					...
					$mt->rtype			= "AR";
					$mt->kode_tagihan 	= "TB-".$menu->id."-".$menu->urutan;
					$mt->id_properti 	=  $kon->id_properti;
					$mt->id_kav 	    =  $kon->id_kav;
					$mt->tanggal 	        = $request->input('tanggal');
					$mt->pembayar 	    = $request->input('nama');
				...
				//=================START INSER JURNAL UMUM===================================
					...
					$ket_jurnal = "Penerimaan Pembayaran Tambahan Tanah/Bangunan :: ".$mproperti->nama." Kav ".$munit->nama." . Dari :".$request->input('nama')." Kode: BOK-".$menu->id."-".$menu->urutan." | ".$request->input("keterangan");
        ...
        return redirect('tambahan/detail/'.$menu->id_spr);
	...
	public function cetak($kode)
    {
		...
		$pdf = PDF::loadview('Tambahan::cetak', ['data'=>$data])->setPaper('a4');
