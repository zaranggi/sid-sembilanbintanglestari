...
<h1 class="page-header">Manage Surat Konsumen<small>...</small></h1>
        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-inverse">
                ...
                <div class="panel-body">
                    <form enctype="multipart/form-data" class="form-horizontal" method="POST" role="form" action="{{ route('suratkonsumen.update', $data->id) }}"> 
                        @csrf
                        @method('PATCH') 
                        @if ( count( $errors ) > 0 ) 
						...
						<div class="form-group m-b-15">
							<label class="col-lg-2 col-form-label">Konsumen</label>
                            <div class="input-group col-lg-6"> 
								 <select name="id_konsumen" class="pilih form-control" placeholder="Pilih Konsumen" required >      
									<option>-- Pilih Konsumen --</option>
									@foreach($konsumen as $r)
										@if($data->id_konsumen == $r->id)
											<option value="{{ $r->id }}" selected>{{ $r->kode }} :: {{ $r->nama }} - {{ $r->nama_properti }}</option>      
										@else 
											<option value="{{ $r->id }}">{{ $r->kode }} :: {{ $r->nama }} - {{ $r->nama_properti }}</option>      
										@endif
									@endforeach
